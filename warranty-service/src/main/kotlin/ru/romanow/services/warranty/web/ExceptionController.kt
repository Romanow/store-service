...
class ExceptionController {
    ...
    @ExceptionHandler(value = [ItemNotOnWarrantyException::class])
    fun conflict(exception: RuntimeException): ErrorResponse {
        return ErrorResponse(exception.message)
    }

    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    @ExceptionHandler(value = [WarehouseProcessException::class])
    fun unprocessable(exception: RuntimeException): ErrorResponse {
        return ErrorResponse(exception.message)
    }
